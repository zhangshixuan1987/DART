function E3SM_DART_correl(datadir,DARTfile,DARTvarname,DARTlevel,E3SMvarname,E3SMlocation)
%% E3SM_DART_correl  explores correlation between one E3SM location and the DART ensemble
%
% datadir     = '/ptmp/raeder/Cam3.6/Hist0';
% DARTfile    = 'Pr_06_ens.nc';
% DARTvarname = 'T';
% DARTlevel   = 600;
% E3SMvarname  = 'EVAPPREC';
% E3SMlocation = [170, 31, 600];  % lon/lat/level
%
% E3SM_DART_correl(datadir,DARTfile,DARTvarname,DARTlevel,E3SMvarname,E3SMlocation)
...
%----------------------------------------------------------------------
% Get the time series from the E3SM files:
...
E3SMlevels = nc_varget(camname,'lev'); E3SMNlevels = length(E3SMlevels);
E3SMlats   = nc_varget(camname,'lat'); E3SMNlats   = length(E3SMlats);
E3SMlons   = nc_varget(camname,'lon'); E3SMNlons   = length(E3SMlons);
E3SMunits  = nc_attget(camname,E3SMvarname,'units'); 
E3SMtimestr = timebase(camname);

diffs  = abs(E3SMlevels - E3SMlocation(3)); [camlevel,camlevind] = min(diffs);
diffs  = abs(E3SMlats   - E3SMlocation(2)); [camlat,  camlatind] = min(diffs);
diffs  = abs(E3SMlons   - E3SMlocation(1)); [camlon,  camlonind] = min(diffs);

camlevel = E3SMlevels(camlevind);
camlat   = E3SMlats(  camlatind);
camlon   = E3SMlons(  camlonind);
...
str1 = sprintf('%s correlated against %s',E3SMvarname,DARTvarname);
str2 = sprintf('%s [%.2f , %.2f] level %.2f',E3SMvarname,camlon,camlat,camlevel);
str3 = sprintf('%s level %.2f',DARTvarname,dartlevel);

subplot('position',posmat(1,:))
plot(x)
title( {str2, E3SMtimestr} )
ylabel(E3SMunits)
...
mymap = [1 1 1; jet];
colormap(mymap)
imagesc(E3SMlons,E3SMlats,cammean)
set(gca,'YDir','normal')
h = colorbar;
set(get(h,'YLabel'),'String',E3SMunits)
worldmap;
axis image
hold on; 
plot([  0 360],[camlat camlat],'k', ...
     [camlon camlon],[-90 90],'k');
title(sprintf('Ensemble mean %s level %.2f %s',E3SMvarname,camlevel,E3SMtimestr))
...
title({str1,str2,str3,E3SMtimestr})
