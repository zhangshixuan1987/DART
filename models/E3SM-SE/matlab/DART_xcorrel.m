...
E3SMunits  = nc_attget(camname,varname1,'units'); 
E3SMtimestr = timebase(camname);

diffs  = abs(E3SMlevels - location(3)); [camlevel,camlevind] = min(diffs);
diffs  = abs(E3SMlats   - location(2)); [camlat,  camlatind] = min(diffs);
diffs  = abs(E3SMlons   - location(1)); [camlon,  camlonind] = min(diffs);
...
title(sprintf('Ensemble mean %s level %.2f   %s',varname1,camlevel,E3SMtimestr))
